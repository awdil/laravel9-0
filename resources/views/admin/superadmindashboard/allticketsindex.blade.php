		...
		@endsection
							...
							<div class="page-header d-xl-flex d-block">
								<div class="page-leftheader">
									<h4 class="page-title"><span class="font-weight-normal text-muted ms-2">{{lang('Manage Complaints', 'menu')}}</span></h4>
								</div>
							</div>
							
							@include('admin.superadmindashboard.ticketfilter')
							<!--- Ticket List Blocks --->
							...
							<div class="col-xl-12 col-lg-12 col-md-12">
								<div class="card">
									<div class="card-header border-0">
										<h4 class="card-title">{{lang('Total Active Complaints', 'menu')}}</h4>
									</div>
												...
												<table
													...
													<tbody id="refresh">
														@foreach($tickets as $ticket)
															<tr {{ $ticket->replystatus == 'Replied' ? 'class=bg-success-transparent' : '' }}>
																<td class="wpx-40 text-center">
																	...
																	@else
																		<input type="checkbox" name="student_checkbox[]" class="checkall" value="{{ $ticket->id }}" disabled />
																	@endif
																</td>
																	...
																	<div class="d-flex align-items-center">
																		<div class="">
																			@if($ticket->ticketnote->isEmpty())
																				@if($ticket->overduestatus != null)
																					<div class="ribbon ribbon-top-right1 text-danger">
																						<span class="bg-danger text-white">{{ $ticket->overduestatus }}</span>
																					</div>
																				@endif
																			@else
																				<div class="ribbon ribbon-top-right text-warning">
																					<span class="bg-warning text-white">{{ __('Note') }}</span>
																				</div>
																				@if($ticket->overduestatus != null)
																					<div class="ribbon ribbon-top-right1 text-danger">
																						<span class="bg-danger text-white">{{ $ticket->overduestatus }}</span>
																			...
																			<ul class="fs-13 font-weight-normal d-flex custom-ul">
																				<li class="pe-2 text-muted">#{{ $ticket->ticket_id }}</li>
																				<li class="px-2 text-muted" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ __('Date') }}"><i class="fe fe-calendar me-1 fs-14"></i> {{ $ticket->created_at->timezone(Auth::user()->timezone)->format(setting('date_format')) }}</li>
																				@if($ticket->priority != null)
																					<li class="ps-5 pe-2 preference preference-{{ strtolower($ticket->priority) }}" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ __('Priority') }}">{{ __($ticket->priority) }}</li>
																				@else
																					~
																				@endif
																				@if($ticket->category_id != null)
																					@if($ticket->category != null)
																						<li class="px-2 text-muted" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ __('Category') }}"><i class="fe fe-grid me-1 fs-14"></i>{{ Str::limit($ticket->category->name, '40') }}</li>
																					@else
																						~
																					@endif
																				@else
																				...
																				@if($ticket->last_reply == null)
																					<li class="px-2 text-muted" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ __('Last Replied') }}"><i class="fe fe-clock me-1 fs-14"></i>{{ $ticket->created_at->diffForHumans() }}</li>
																				@else
																					<li class="px-2 text-muted" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ __('Last Replied') }}"><i class="fe fe-clock me-1 fs-14"></i>{{ $ticket->last_reply->diffForHumans() }}</li>
																				@endif
																			</ul>
																		</div>
																	</div>
																</td>
																<td>{{ $ticket->cust->username }}</td>
																<td>
																		...
																		@if($ticket->status == 'Suspend' || $ticket->status == 'Closed')
																			<div class="btn-group">
																				@if($ticket->ticketassignmutliples->isNotEmpty() && $ticket->selfassignuser_id == null)
																					<button class="btn btn-outline-primary dropdown-toggle" data-bs-toggle="dropdown" disabled>{{ __('Multi Assign') }} <span class="caret"></span></button>
																					<button data-id="{{ $ticket->id }}" class="btn btn-outline-primary" id="btnremove" disabled data-bs-toggle="tooltip" data-bs-placement="top" title="{{ __('Unassign') }}" aria-label="Unassign"><i class="fe fe-x"></i></button>
																				@elseif($ticket->ticketassignmutliples->isEmpty() && $ticket->selfassignuser_id != null)
																					...
																					<button data-id="{{ $ticket->id }}" class="btn btn-outline-primary" id="btnremove" disabled data-bs-toggle="tooltip" data-bs-placement="top" title="{{ __('Unassign') }}" aria-label="Unassign"><i class="fe fe-x"></i></button>
																				@else
																					<button class="btn btn-outline-primary dropdown-toggle" data-bs-toggle="dropdown" disabled>{{ __('Assign') }}<span class="caret"></span></button>
																				@endif
																			</div>
																		@else
																			@if($ticket->ticketassignmutliples->isEmpty() && $ticket->selfassignuser_id == null)
																				<div class="btn-group">
																					<button class="btn btn-outline-primary dropdown-toggle btn-sm" data-bs-toggle="dropdown">{{ __('Assign') }} <span class="caret"></span></button>
																					<ul class="dropdown-menu" role="menu">
																						<li class="dropdown-plus-title">{{ __('Assign') }} <b aria-hidden="true" class="fa fa-angle-up"></b></li>
																						<li>
																							<a href="javascript:void(0);" id="selfassigid" data-id="{{ $ticket->id }}">{{ __('Self Assign') }}</a>
																						</li>
																						<li>
																							<a href="javascript:void(0)" data-id="{{ $ticket->id }}" id="assigned">{{ __('Other Assign') }}</a>
																						</li>
																					</ul>
																				</div>
																			@else
																				<div class="btn-group">
																					@if($ticket->ticketassignmutliples->isNotEmpty() && $ticket->selfassignuser_id == null)
																						@if($ticket->ticketassignmutliples->isEmpty() && $ticket->selfassign == null)
																							<button class="btn btn-outline-primary dropdown-toggle btn-sm" data-bs-toggle="dropdown">{{ __('Assign') }} <span class="caret"></span></button>
																						@else
																							<button class="btn btn-outline-primary dropdown-toggle btn-sm" data-bs-toggle="dropdown">{{ __('Multi Assign') }} <span class="caret"></span></button>
																							<a href="javascript:void(0)" data-id="{{ $ticket->id }}" class="btn btn-outline-primary btn-sm" id="btnremove" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ __('Unassign') }}" aria-label="Unassign"><i class="fe fe-x"></i></a>
																						@endif
																					@elseif($ticket->ticketassignmutliples->isEmpty() && $ticket->selfassignuser_id != null)
																						@if($ticket->ticketassignmutliples->isEmpty() && $ticket->selfassign == null)
																							<button class="btn btn-outline-primary dropdown-toggle btn-sm" data-bs-toggle="dropdown">{{ __('Assign') }} <span class="caret"></span></button>
																...
																<td>
																	@if(Auth::user()->can('Ticket Edit'))
																		<a href="{{ url('admin/ticket-view/' . $ticket->ticket_id) }}" class="btn btn-sm action-btns edit-testimonial"><i class="feather feather-eye text-primary" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ __('Edit') }}"></i></a>
																	@else
																		~
																	@endif
																	@if(Auth::user()->can('Ticket Delete'))
																		<a href="javascript:void(0)" data-id="{{ $ticket->id }}" class="btn btn-sm action-btns" id="show-delete"><i class="feather feather-trash-2 text-danger" data-id="{{ $ticket->id }}" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ __('Delete') }}"></i></a>
																	@else
																		~
																	@endif
																</td>
